...
class ProductionManagementAgent {
  ...
  async processContent(contentData) {
    try {
      ...
      const productionData = {
        ...
        // Narration language (from the content matrix via strategy) — must be
        // set before asset generation since TTS reads it for voice selection.
        language: strategy?.language || 'en',
        languageName: strategy?.languageName || 'English',
        status: 'processing',
        assets: {
  ...
  async generateAudioNarration(productionData) {
    ...
    try {
      ...
      // Language-aware TTS: the strategy language decides the voice (Google
      // web TTS supports all matrix languages; cloud fallbacks are native).
      const language = productionData.language || productionData.strategy?.language || 'en';
      await this.aiVideoGenerator.generateTTSAudio(ttsText, audioPath, { language });
  ...
  async generateCaptions(productionData) {
    ...
    productionData.assets.captions = {
      path: captionsPath,
      format: 'srt',
      language: productionData.language || productionData.script?.metadata?.strategy?.language || 'en',
      autoGenerated: true
    };
